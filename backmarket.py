...
with open('user_urls.json', 'a+') as user_urls_file:
...
# Load user URLs from 'user_urls.json'
def load_user_urls():
    try:
        with open('user_urls.json', 'r') as json_file:
...
def scrape_product_info(url, use_proxies, proxy_list):
    try:
        ...
        # Find the structured data script element
        script_element = soup.find('script', type='application/ld+json')

        if script_element:
            # Extract the structured data as a dictionary
            structured_data = json.loads(script_element.get_text())

            # Extract the title
            title = structured_data.get('name', '')

            # Attempt to extract the price and availability
            offers = structured_data.get('offers', {})
            price = offers.get('price', 'Out of stock')
            availability = offers.get('availability', 'Out of stock')

            # Convert availability to "In stock" or "Out of stock"
            availability = "In stock" if availability == "http://schema.org/InStock" else "Out of stock"
        ...
        # Remove the print statement for errors
        # print(f"Error: {e}")
        return None

# Function to listen for user input and add valid URLs to 'user_urls.json'
def add_url_to_user_urls(url):
    if 'backmarket.com' in url:
        user_urls = load_user_urls()
        if url not in user_urls:
            user_urls.append(url)
            with open('user_urls.json', 'w') as json_file:
...
def scrape_and_update_product_data(use_proxies, proxy_list):
    ...
    # Load existing product data from 'products.json' if it exists
    try:
